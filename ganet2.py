...
class AuxiliaryNet(torch.nn.Module):
    ...
    def __init__(self, hidden_dim, cut_length, max_length, tau=0.1):
...
def train(model, iterator, optimizer, criterion):
    ...
    for batch in tqdm(iterator):
        ...
        predictions1 = predictions.view((N_CIRCLE,-1,2)).permute((1,0,2))
        ...
        acc_1 = sum(torch.gt(acc_0,0.5))
        # acc =  torch.div(acc_1,len(acc_0)) 

        epoch_loss += loss.item()
        epoch_acc += acc_1.item()/len(acc_0)
